    ...
    with st.container():
        ...
        if st.session_state.show_members_form:
             with st.form(key="add_story"):         
                ...
                if submit_button:
                    if upload_picture:
                        logging.debug("File uploaded, converting to URL")
                        file_url = convert_file(upload_picture)
                        if file_url:
                            logging.debug(f"File URL: {file_url}")
                            story = generate_story(st.session_state.username, file_url)
                            st.session_state.add_member_status = (True, story)
                        else:
                            st.session_state.add_member_status = (False, "Failed to upload image.")
                        st.session_state.show_members_form = False
                        st.rerun()
                    else:
